...
export async function PUT(request: Request, context: Context) {
  try {
    ...
    if (db.prepare("SELECT id FROM visits WHERE report_no = ? AND id <> ?").get(reportNo, id)) {
    ...
    db.transaction(() => {
      ...
      for (const productId of input.productIds) attach.run(id, productId);
    })();
...
export async function DELETE(_request: Request, context: Context) {
  try {
    ...
    db.prepare("UPDATE visits SET deleted_at = datetime('now'), updated_at = datetime('now') WHERE id = ?").run(id);
