 ...
 *
 * 默认会播种演示账号与演示业务数据（开发、试用都靠它）。
 * 交付给客户的库不能带这些：Electron 打包版启动服务时传 CRM_SEED_DEMO=0，
 * 只建管理员账号（admin / Admin@123）与标签字典，业务数据由用户自己导入表格产生。
 */
export function seedDatabase(db: Database.Database) {
  const withDemo = process.env.CRM_SEED_DEMO !== "0";
  const seed = db.transaction(() => {
    ...
    // 演示业务数据：客户 / 产品 / 联系人 / 拜访 / 商机 / 订单，交付客户的库不要这些
    if (withDemo) {
